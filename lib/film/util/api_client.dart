...
import 'package:kdf/film/model/episode.dart';
...
import 'package:kdf/film/util/constants.dart';
...
class ApiClient {
  ...
  Future<List<MediaItem>> fetchMovies(
    ...
    var url = Uri.https(baseUrl, '3/movie/$category',
        {'api_key': API_KEY,'language': 'tr-TR','append_to_response':'images&include_image_language=tr', 'page': page.toString()});
  ...
  Future<List<MediaItem>> getSimilarMedia(int mediaId,
    ...
    var url = Uri.https(baseUrl, '3/$type/$mediaId/similar', {
      'api_key': API_KEY,
      'language': 'tr-TR',
      'append_to_response':'images&include_image_language=tr'
    });
  ...
  Future<List<Actor>> getMediaCredits(int mediaId,
      {String type: "movie"}) async {
    var url =
        Uri.https(baseUrl, '3/$type/$mediaId/credits', {'api_key': API_KEY,'language': 'tr-TR','append_to_response':'images&include_image_language=tr'});
  ...
  Future<dynamic> getMediaDetails(int mediaId, {String type: "movie"}) async {
    var url = Uri.https(baseUrl, '3/$type/$mediaId', {'api_key': API_KEY,'language': 'tr-TR','append_to_response':'images&include_image_language=tr'});
  ...
  Future<List<SearchResult>> getSearchResults(String query) {
    var url = Uri
        .https(baseUrl, '3/search/multi', {'api_key': API_KEY, 'language': 'tr-TR','append_to_response':'images&include_image_language=tr', 'query': query});
