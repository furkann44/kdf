...
class BookDatabase {
  ...
  Future _init() async {
    ...
    String path = join(documentsDirectory.path, "kdf.db");
        ...
        onCreate: (Database db, int version) async {
          ...
          await db.execute(
              "CREATE TABLE $tableName ("
                  ...
                  "${Book.db_pageCount} INTEGER,"
                  "${Book.db_buyLink} TEXT,"
                  "${Book.db_webReaderLink} TEXT,"
                  "${Book.db_isbn} TEXT"
                  ")");
        });
    didInit = true;
  ...
  Future updateBook(Book book) async {
    ...
    await db.rawInsert(
        'INSERT OR REPLACE INTO '
            '$tableName(${Book.db_id}, ${Book.db_title}, ${Book.db_url}, ${Book.db_star}, ${Book.db_notes}, ${Book.db_author}, ${Book.db_description}, ${Book.db_subtitle}, ${Book.db_pageCount}, ${Book.db_buyLink}, ${Book.db_webReaderLink}, ${Book.db_isbn})'
            ' VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [book.id, book.title, book.url, book.starred? 1:0, book.notes, book.author, book.description, book.subtitle, book.pageCount, book.buyLink, book.webReaderLink, book.isbn]);
